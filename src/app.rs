...
pub struct SerialReaderApp {
    connection: ConnectionConfig
}

impl SerialReaderApp {
        ...
        egui::SidePanel::left("ConfigPanel")
            ...
            .show(ctx, |ui| {
                ...
                frame.show(ui, |ui| {
                    ...
                    option_dropdown(ui, "Device", DEVICES, &mut self.connection.port.as_str(), 20.0);
                    option_dropdown(ui, "Baud", BAUD_RATES, &mut self.connection.baud_rate, 27.0);
                    ui.separator();

                    option_dropdown(ui, "Data bits", DATA_BITS, &mut self.connection.data_bits, 8.0);
                    option_dropdown(ui, "Parity", PARITIES, &mut self.connection.parity, 24.5);
                    option_dropdown(ui, "Stop bits", STOP_BITS, &mut self.connection.stop_bits, 8.0);
                    option_dropdown(ui, "Flow ctrl", FLOW_CTRLS, &mut self.connection.flow_ctrl, 10.0);
                    ui.separator();

                    option_dropdown(ui, "DTR", &[false, true], &mut self.connection.dtr, 33.0);
                    option_dropdown(ui, "Start mode", START_MODES, &mut self.connection.start_mode, -6.0);
                    if matches!(self.connection.start_mode, StartMode::Delay(_)) {
                        ui.horizontal(|ui| {
                            ui.label("Delay (ms)");
                            ui.add_space(0.0);
                            ui.add(egui::DragValue::new(&mut self.connection.start_delay).clamp_range(0..=100000).max_decimals(0));
                        });
                    }
                    else if matches!(self.connection.start_mode, StartMode::Message(_)) {
                        ui.horizontal(|ui| {
                            ui.label("Message");
                            ui.add_space(7.0);
                            ui.add(egui::TextEdit::singleline(&mut self.connection.start_msg).desired_width(DROPDOWN_WIDTH - ui.style().spacing.item_spacing.x));
                        });
                    }
                });
            });
...
impl eframe::App for SerialReaderApp {
